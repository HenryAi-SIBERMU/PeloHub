...
const Sidebar: React.FC<SidebarProps> = ({ isOpen = false, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`flex flex-col w-64 h-screen bg-white dark:bg-card-dark border-r border-gray-200 dark:border-gray-800 fixed left-0 top-0 z-50 transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Logo Area */}
        ...
        <nav className="flex-1 overflow-y-auto py-6 flex flex-col gap-1">
          ...
          <Link
            to="/build-engine"
            className={`flex items-center gap-3 px-6 py-3 text-sm font-medium transition-colors ${isActive('/build-engine')}`}
          >
            <span className="material-symbols-outlined text-[20px]">construction</span>
            <span className="flex-1">{t('sidebar.build_engine')}</span>
            <span className="px-2 py-0.5 text-[10px] font-bold bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 rounded-full uppercase tracking-wide">Soon</span>
          </Link>
